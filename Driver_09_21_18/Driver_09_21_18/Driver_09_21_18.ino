...
double speedToDelay(float reverse_coeffs[], double Speed, double angle) {
    /* convert float array to double via casting */
...
double findAngle(long x1, long y1, long x0, long y0){ /*inputs: final x destination; final y destination, initial x location, ...
 * initial y location
 */
    long dispx = x1 - x0;
    long dispy = y1 - y0;
        ...
        switch ((int) *command) { /*switch case based on first command*/
            ...
            case 2: // moveTo:x0:y0:hold duration:speed
                ...
                */
                ...
                long xfinal;
                xfinal = (*(command +1)); /* destination in x-direction */
                long yfinal;
                yfinal = (*(command +2)); /* destination in y-direction */

                /* Find angle of tangent between two points*/
                double ang;
                ang = findAngle(location[0], location[1], xfinal, yfinal);
